void oscEvent(OscMessage msg) {
  ...
  if (msg.checkAddrPattern("/newEvent")==true) {
    ...
    //PVector translation = PVector.random3D().mult(500);
    PVector translation = PVector.random3D();
    translation.x *= width;
    translation.y *= height;
    translation.z *= 500;
    for (int i = 0; i < numJets; i++) {
      events[nextEventIndex].jets[i] = new Jet(numConstituents[i], translation);
    }
  ...
  if (msg.checkAddrPattern("/eventUsed") == true) {
    ...
    for(int i = 0; i < numElementsUsed; i++) {
      ...
      ind += 2;
      if(constNum == -1) {
        eventUsed.jets[jetNum].spin();
      } else {
        eventUsed.jets[jetNum].particles[constNum].flash();
      }
    }
  }
